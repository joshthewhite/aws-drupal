...
OptionParser.new do |opts|
  ...
  opts.on('-e', '--email [EMAIL]', 'Alert email') do |v|
    options[:email] = v
  end
end.parse!
...
archive_path = "./puppet/#{archive}"
File.delete(archive_path) if File.exist?(archive_path)
zf = ZipFileGenerator.new('./puppet/content', archive_path)
...
config_files.each do |file|
  ...
  s3_obj.write(Pathname.new("./puppet/#{file}"), acl: :public_read)
end
...
master_template = File.open('puppet-master.template', 'rb').read
...
# Grab the Cloud Formation templates used to create the stacks.
drupal_template = File.open('puppet-drupal.template', 'rb').read

# Collect the alert email address from the user.
alert_email = options[:email] ? options[:email] : prompt('Enter alert email: ')
